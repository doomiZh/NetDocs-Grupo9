...
<xsl:stylesheet version="1.0"
    ...
    <html lang="{@lang}">
      <head>
        ...
        <style>
          ...
          nav{position:sticky;top:12px;background:#fffbf7;border:1px solid #e8d5c4;border-radius:12px;padding:12px}
          nav h3{margin:0 0 8px;font-size:13px;color:#9a3412;text-transform:uppercase;letter-spacing:.5px}
          nav a{display:block;padding:6px 10px;margin:3px 0;border-radius:8px;color:#7c2d12;text-decoration:none;font-size:13px}
          nav a:hover{background:#fed7aa}
          section{background:#fffbf7;border:1px solid #e8d5c4;border-radius:12px;padding:16px;margin:0 0 14px}
          ...
          .pill{display:inline-block;background:#fff7ed;border:1px solid #fdba74;color:#c2410c;padding:1px 8px;border-radius:99px;font-size:11px;margin-right:6px;font-weight:600}
          strong{color:#7c2d12}
      ...
      <body>
        <header>
          <div class="wrap">
            <h1><xsl:value-of select="metadata/titulo"/></h1>
            <p><xsl:value-of select="metadata/subtitulo"/></p>
          </div>
        </header>
        <div class="wrap grid">
          <nav>
            ...
            <xsl:for-each select="secciones/seccion">
              <a href="#{@id}"><xsl:value-of select="@nombre"/></a>
            </xsl:for-each>
          </nav>
          <main><xsl:apply-templates select="secciones/seccion"/></main>
        </div>
      </body>
    </html>
  </xsl:template>
  ...
  <xsl:template match="seccion">
    <section id="{@id}">
      <h2><span class="pill"><xsl:value-of select="@id"/></span> <xsl:value-of select="@nombre"/></h2>
  ...
  <xsl:template match="parrafo[@tipo='definicion']">
    <p class="def"><strong>Definición:</strong> <xsl:value-of select="."/></p>
  </xsl:template>
  <xsl:template match="parrafo[@tipo='nota']">
    <p class="nota"><xsl:value-of select="."/></p>
  </xsl:template>
  ...
  <xsl:template match="lista">
    <xsl:if test="@titulo"><p><strong><xsl:value-of select="@titulo"/>:</strong></p></xsl:if>
    ...
    <div class="cards">
      <xsl:for-each select="tarjeta">
        <div class="card">
          ...
          <xsl:if test="@sub"><div class="sub"><xsl:value-of select="@sub"/></div></xsl:if>
          <xsl:apply-templates/>
        </div>
      </xsl:for-each>
    </div>
  ...
  <xsl:template match="tabla">
    <xsl:if test="@titulo"><h4 style="margin:10px 0 4px;font-size:14px"><xsl:value-of select="@titulo"/></h4></xsl:if>
    <table>
      <thead><tr><xsl:for-each select="col"><th><xsl:value-of select="@nombre"/></th></xsl:for-each></tr></thead>
      <tbody><xsl:for-each select="fila"><tr><xsl:for-each select="c"><td><xsl:value-of select="."/></td></xsl:for-each></tr></xsl:for-each></tbody>
    </table>
  </xsl:template>
